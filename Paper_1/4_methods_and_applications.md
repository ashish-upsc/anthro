...
```mermaid
graph TD
    A[Holistic Spirit of Anthropology] --> B[1. Physical/Biological Anthropology]
    A --> C[2. Socio-Cultural Anthropology]
    A --> D[3. Archaeological Anthropology]
    A --> E[4. Linguistic Anthropology]
    
    B -->|Connects to| F[Holism: Overlapping biocultural synthesis]
    C -->|Connects to| F
    D -->|Connects to| F
    E -->|Connects to| F
```
...
```mermaid
graph LR
    A[Armchair Anthropology: 19th Century] -->|Secondary Reports| B[Speculative conjectures / Tylor & Frazer]
    B --> C[Veranda Anthropology: Early 20th Century]
    C -->|Interviews on porch| D[Haddon's Torres Straits Expedition, 1898]
    D --> E[Participant Observation Revolution: 1922]
    E -->|Living with natives| F[Malinowski Trobriand Islands Fieldwork]
    F --> G[Contemporary Reflexive / Postmodern Fieldwork]
```
...
```mermaid
graph TD
    A[Forensic Skeletal Profiling] --> B[1. Biological Sex Determination]
    A --> C[2. Age-at-Death Estimation]
    A --> D[3. Stature Reconstruction]
    A --> E[4. Facial Reconstruction]
    
    B -->|Diagnostic Area| B1[Pelvis: Pubic angle, sciatic notch]
    C -->|Diagnostic Area| C1[Teeth eruption, epiphyseal fusion of bones]
    D -->|Diagnostic Area| D1[Long bones: Femur, humerus length]
    E -->|Diagnostic Area| E1[Clay/Digital mapping over skull landmarks]
```
