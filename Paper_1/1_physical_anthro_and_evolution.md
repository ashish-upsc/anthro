...
```mermaid
graph TD
    A[Environmental Change] --> B[New Needs / Desires]
    B --> C[Use or Disuse of Organs]
    C -->|Use: Strengthens Organ| D[Acquired Character]
    C -->|Disuse: Weakens/Atrophies Organ| D
    D -->|Inherited by| E[Offspring / Next Generation]
    E --> F[Evolution of New Species]
```
...
```mermaid
graph TD
    A[Rapid Multiplication / Overproduction] --> B[Limited Resources: Food & Space]
    B --> C[Stray Struggle for Existence]
    C --> D[Organic Variations in Population]
    D -->|Favorable Variations| E[Survival of the Fittest / Natural Selection]
    D -->|Unfavorable Variations| F[Extinction]
    E -->|Inheritance of Variations| G[Origin of New Species over Generations]
```
...
```mermaid
graph TD
    A[Forces of Synthetic Evolution] --> B[1. Genetic Variation Producers]
    A --> C[2. Directional Evolutionary Forces]
    
    B --> B1[Gene Mutation]
    B --> B2[Chromosomal Aberrations]
    B --> B3[Genetic Recombination]
    
    C --> C1[Natural Selection]
    C --> C2[Genetic Drift / Sewall Wright Effect]
    C --> C3[Gene Flow / Migration]
    C --> C4[Reproductive Isolation]
```
...
```mermaid
graph TD
    A[Primate Evolutionary Tendencies] --> B[1. Limbs & Hands: Grasping]
    A --> C[2. Senses & Brain: Visual Dominance]
    A --> D[3. Life History: Prolonged Development]
    
    B --> B1[Pentadactyly: 5 digits on hands/feet]
    B --> B2[Opposable Thumb & Big Toe]
    B --> B3[Flat Nails instead of claws]
    B --> B4[Tactile pads with sensory nerve ends]
    
    C --> C1[Stereoscopic/Binocular Vision]
    C --> C2[Color Vision]
    C --> C3[Reduced Olfaction / Snout Shortening]
    C --> C4[Large Brain-to-Body Encephalization]
    
    D --> D1[Single births dominant]
    D --> D2[Extended gestation & infant dependency]
```
...
```mermaid
graph TD
    subgraph QA ["Quadrupedal Ape Skeletal Profile"]
        A[Posterior Foramen Magnum]
        B[C-Shaped Spine]
        C[Long, Narrow Pelvis]
        D[Straight Femur - No carrying angle]
        E[Flat Foot with Opposable Big Toe]
    end
    subgraph BH ["Bipedal Human Skeletal Profile"]
        F[Inferior/Centered Foramen Magnum]
        G[S-Shaped Spine - Lordosis]
        H[Short, Broad, Bowl Pelvis]
        I[Angled Femur - Valgus carrying angle]
        J[Arched Foot with Adducted Big Toe]
    end
```
...
```mermaid
graph TD
    A[Mendelian Inheritance Modes] --> B[1. Autosomal Inheritance]
    A --> C[2. Sex-Linked Inheritance]
    
    B --> B1[Autosomal Dominant]
    B --> B2[Autosomal Recessive]
    
    C --> C1[Sex-Linked Recessive: X-Linked]
    C --> C2[Sex-Linked Dominant: X-Linked]
    C --> C3[Y-Linked / Holandric]
```
...
```mermaid
graph TD
    A[Standard Pedigree Symbols] --> B[Square = Male]
    A --> C[Circle = Female]
    A --> D[Filled Symbol = Affected Individual]
    A --> E[Horizontal Line = Marriage/Mating]
    A --> F[Vertical Line descending from marriage = Offspring]
```
...
| :--- | :--- | :--- | :--- |
| **1. Cross-Sectional Method** | Studying different cohorts of children of different ages at a **single point in time** (e.g., measuring 5-year-olds, 6-year-olds, and 7-year-olds in 2026). | * Extremely fast to execute.<br>* Highly cost-effective.<br>* Allows for massive sample sizes, yielding robust population growth standards. | * Cannot measure individual **growth velocity** (speed of growth).<br>* Mask individual variations, catch-up growth, or the exact onset of the adolescent growth spurt. |
| **2. Longitudinal Method** | Repeatedly studying and measuring the **same cohort of children** over many years (e.g., measuring child A every year from birth to age 18). | * The only method to calculate accurate **growth velocity**.<br>* Reveals precise individual milestones, catch-up growth, and growth spurts. | * Extremely time-consuming (requires 18 years).<br>* Highly expensive.<br>* High **sample attrition** (loss of subjects due to migration, death, or withdrawal). |
| **3. Mixed Longitudinal Method** | Combines both designs. Studies overlapping age cohorts over a shorter time span (e.g., studying Group A from age 5–10, and Group B from age 10–15 simultaneously for 5 years). | * Achieves longitudinal depth in a fraction of the time (e.g., 5 years instead of 15).<br>* Balances cost and time while preserving the ability to calculate growth velocity. | * Requires complex statistical smoothing to merge the overlapping data boundaries. |
...
```mermaid
graph TD
    A[Ecological Morphological Rules] --> B[1. Bergmann's Rule]
    A --> C[2. Allen's Rule]
    
    B -->|Colder Climates| B1[Larger Body Size / Stocky Build]
    B1 -->|Why| B2[Decreases Surface-to-Volume Ratio to conserve heat]
    
    C -->|Colder Climates| C1[Shorter Limbs & Appendages]
    C1 -->|Why| C2[Reduces exposed surface area to minimize heat loss]
```
...
```mermaid
graph TD
    A[ACTN3 R577X Polymorphism] --> B[1. RR Genotype / Speed-Power]
    A --> C[2. XX Genotype / Endurance]
    A --> D[3. RX Genotype / Intermediate]
    
    B -->|Mechanism| B1[High Alpha-Actinin-3 in fast-twitch fibers]
    B1 -->|Athletic Edge| B2[Elite Sprinters, Jumpers, Powerlifters]
    
    C -->|Mechanism| C1[Nonsense mutation: zero Alpha-Actinin-3]
    C1 -->|Athletic Edge| C2[Elite Marathoners, Ultra-Endurance Cyclists]
```
