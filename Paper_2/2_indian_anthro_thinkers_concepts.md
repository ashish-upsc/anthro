...
```mermaid
graph TD
    A[Karve's Kinship Zones] --> B[1. Northern Zone]
    A --> C[2. Southern Zone]
    A --> D[3. Eastern Zone]
    
    B -->|Rules| B1[Sanskrit Indo-Aryan; Strict Lineage Exogamy; Consanguineous marriage banned]
    C -->|Rules| C1[Dravidian family; Preferential Cross-Cousin marriage; Uncle-Niece marriage allowed]
    D -->|Rules| D1[Austro-Asiatic Munda; Matrilineal traces; Complex clan exogamy]
```
...
```mermaid
graph LR
    A[Low Caste / Tribal Group] -->|Adopts twice-born rituals| B[Vegetarianism, Teetotalism, Sacred Thread]
    B -->|Abandons impure habits| C[Pork eating, animal sacrifice, liquor]
    C -->|Claims higher status| D[Assigned higher Jati rank after 1-2 generations]
```
...
```mermaid
graph TD
    A[The Sacred Complex] --> B[1. Sacred Geography]
    A --> C[2. Sacred Performances]
    A --> E[3. Sacred Specialists]
    
    B -->|Composed of| B1[Temples, sacred trees, rivers, shrines]
    C -->|Composed of| C1[Prayers, sacrifices, rituals, pinda-dan]
    E -->|Composed of| E1[Priests, Gayawal Brahmins, shrine keepers]
```
...
```mermaid
graph TD
    subgraph MTE ["Maler Tribe Ecosystem"]
        A["NATURE: Rajmahal Hills, forest, slash-and-burn cultivation (Kurwa)"]
        B["MAN: Social structure, family, tribal council"]
        C["SPIRIT: Demands blood sacrifices, ancestral ghosts (Gossaiyan)"]
        A <--> B
        B <--> C
        C <--> A
    end
