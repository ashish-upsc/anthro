...
```mermaid
graph TD
    A[Indian Neolithic Regional Complexes] --> B[1. Northern Complex: Kashmir]
    A --> C[2. Southern Complex: Deccan]
    A --> D[3. Eastern/Northeastern Complex]
    
    B -->|Sites| B1[Burzahom, Gufkral]
    B -->|Features| B2[Pit-dwellings, bone tools, dog burials]
    
    C -->|Sites| C1[Utnur, Kupgal, Tekkalakota]
    C -->|Features| C2[Ashmounds from burned cattle dung, pastoralism]
    
    D -->|Sites| D1[Chirand, Daojali Hading]
    D -->|Features| D2[Shouldered celts, cord-marked pottery]
```
...
```mermaid
graph LR
    A[Hakra / Early Ravi Phase: 3300-2800 BC] -->|Gradual Evolution| B[Early Harappan: Kot Diji, Amri]
    B -->|Urbanization & Standardization| C[Mature Harappan: Harappa, Mohenjodaro, Rakhigarhi]
    C -->|Decline & Regionalization| D[Late Harappan: Cemetery H, Rangpur]
```
...
```mermaid
graph TD
    A[Indian Social Blueprint] --> B[Varnashrama]
    A --> C[Purushartha]
    A --> D[Karma & Rebirth]
    
    B -->|Life Stages| B1[Brahmacharya -> Grihastha -> Vanaprastha -> Sannyasa]
    C -->|Life Pursuits| C1[Dharma -> Artha -> Kama -> Moksha]
```
