...
```mermaid
graph LR
    A[Segmentary Pole: TRIBE] <==============================> B[Organic Pole: CASTE]
    A -->|Bailey's Criteria: Direct control of land / Egalitarian| B
    B -->|Bailey's Criteria: Hierarchical organic specialization| A
```
...
```mermaid
graph TD
    A[Tribal Development Paradigms] --> B[1. Isolationist / National Park]
    A --> C[2. Assimilationist / Integration]
    A --> D[3. Nehru's Middle Path / Panchsheel]
    
    B -->|Proponent| B1[Verrier Elwin]
    B -->|Strategy| B2[Isolate tribes in protected reserves to prevent exploitation]
    
    C -->|Proponent| C1[G.S. Ghurye]
    C -->|Strategy| C2[Treat tribes as 'Backward Hindus'; integrate into mainstream caste system]
    
    D -->|Proponent| D1[Jawaharlal Nehru]
    D -->|Strategy| D2[Respect tribal land/forest rights; let them progress at their own pace]
```
